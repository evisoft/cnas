using Cnas.Ps.Core.Common;
using Cnas.Ps.Infrastructure.Documents.Templates;
using DocumentFormat.OpenXml.Packaging;
using DocumentFormat.OpenXml.Wordprocessing;

namespace Cnas.Ps.Infrastructure.Tests.Documents.Templates;

/// <summary>
/// Unit tests for <see cref="CerereDocumenteLipsaTemplate"/> — the Annex 7
/// "Cerere documente lipsă" (request for missing documents) template.
/// </summary>
public class CerereDocumenteLipsaTemplateTests
{
    /// <summary>Reference UTC deadline used across the suite.</summary>
    private static readonly DateTime DeadlineUtc = new(2026, 7, 15, 0, 0, 0, DateTimeKind.Utc);

    [Fact]
    public void TemplateCode_Property_MatchesExpected()
    {
        var template = new CerereDocumenteLipsaTemplate();

        template.TemplateCode.Should().Be(CerereDocumenteLipsaTemplate.Code);
        template.TemplateCode.Should().Be("cerere-documente-lipsa");
    }

    [Fact]
    public void Render_HappyPath_ProducesValidDocx()
    {
        var template = new CerereDocumenteLipsaTemplate();

        var result = template.Render(HappyPathFacts());

        result.IsSuccess.Should().BeTrue();
        result.Value.Length.Should().BeGreaterThan(4);
        // ZIP local-file-header signature — DOCX is a ZIP envelope.
        result.Value[0].Should().Be(0x50);
        result.Value[1].Should().Be(0x4B);
        result.Value[2].Should().Be(0x03);
        result.Value[3].Should().Be(0x04);

        using var ms = new MemoryStream(result.Value);
        using var doc = WordprocessingDocument.Open(ms, isEditable: false);
        var paragraphs = doc.MainDocumentPart!.Document.Body!.Descendants<Paragraph>().ToList();
        paragraphs.Should().NotBeEmpty();
    }

    [Fact]
    public void Render_RendersRecipientAndMissingDocsAndDeadline()
    {
        var template = new CerereDocumenteLipsaTemplate();
        var missingDocs = new List<string>
        {
            "Copia certificatului de naștere (Serviciul Stare Civilă)",
            "Adeverința de la locul de muncă",
            "Extras din cartea de muncă",
        };
        var facts = HappyPathFacts();
        facts["missingDocs"] = missingDocs;

        var result = template.Render(facts);

        result.IsSuccess.Should().BeTrue();
        var text = DeciziaPensieTemplateTests.ExtractAllText(result.Value);
        text.Should().Contain("Ion Popescu");
        text.Should().Contain("mun. Chișinău, str. Pacii 10/2");
        foreach (var d in missingDocs)
        {
            text.Should().Contain(d);
        }
        // Deadline as UTC date.
        text.Should().Contain("2026-07-15");
    }

    [Fact]
    public void Render_MissingFact_ReturnsTemplateMissingFacts()
    {
        var template = new CerereDocumenteLipsaTemplate();
        var facts = HappyPathFacts();
        facts.Remove("beneficiaryFullName");

        var result = template.Render(facts);

        result.IsFailure.Should().BeTrue();
        result.ErrorCode.Should().Be(ErrorCodes.TemplateMissingFacts);
        result.ErrorMessage.Should().Contain("beneficiaryFullName");
    }

    [Fact]
    public void Render_MissingMissingDocs_ReturnsTemplateMissingFacts()
    {
        var template = new CerereDocumenteLipsaTemplate();
        var facts = HappyPathFacts();
        facts.Remove("missingDocs");

        var result = template.Render(facts);

        result.IsFailure.Should().BeTrue();
        result.ErrorCode.Should().Be(ErrorCodes.TemplateMissingFacts);
        result.ErrorMessage.Should().Contain("missingDocs");
    }

    /// <summary>Builds a fact dictionary that satisfies all required keys.</summary>
    private static Dictionary<string, object?> HappyPathFacts() => new()
    {
        ["beneficiaryFullName"] = "Ion Popescu",
        ["beneficiaryAddress"] = "mun. Chișinău, str. Pacii 10/2",
        ["dossierSqid"] = "SQID-DOSS-77",
        ["missingDocs"] = new List<string> { "Copia buletinului", "Adeverința de la angajator" },
        ["deadlineUtc"] = DeadlineUtc,
    };
}
